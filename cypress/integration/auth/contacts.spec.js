...
describe("contacts", () => {
  ...
  it("should add contact", () => {
    ...
    cy.get("app-contacts ion-buttons ion-button").click();

    cy.get('app-contacts-add input[name="saerch"]').type("c.gdynia@live.de");
    // .should("have.value", "c.gdynia@live.de");

    cy.get('ion-buttons ion-button [type="submit"]').click();
    cy.contains("button", "Add").click(); //ansprechen unklar muss noch gemacht werden!!!!!!!
  });
});
